 ...
 * @date    2023
 * @brief   Python bindings for
 * [whisper.cpp](https://github.com/ggerganov/whisper.cpp) using Pybind11
 *
 * @par
...
struct whisper_context_wrapper
whisper_init_from_file_wrapper(const char *path_model) {
  struct whisper_context *ctx = whisper_init_from_file_with_params(
      path_model, whisper_context_default_params());
...
struct whisper_context_wrapper
whisper_init_from_buffer_wrapper(void *buffer, size_t buffer_size) {
  struct whisper_context *ctx = whisper_init_from_buffer_with_params(
      buffer, buffer_size, whisper_context_default_params());
  struct whisper_context_wrapper ctw_w;
  ctw_w.ptr = ctx;
  return ctw_w;
}
...
struct whisper_context_wrapper
whisper_init_wrapper(struct whisper_model_loader_wrapper *loader) {
  struct whisper_context *ctx =
      whisper_init_with_params(loader->ptr, whisper_context_default_params());
  struct whisper_context_wrapper ctw_w;
  ctw_w.ptr = ctx;
  return ctw_w;
};
...
int whisper_set_mel_wrapper(
    whisper_context_wrapper *ctx,
    nb::ndarray<nb::numpy, float, nb::shape<-1>, nb::c_contig> data, int n_len,
    int n_mel) {
...
whisper_get_logits_wrapper(whisper_context_wrapper *ctx_w, size_t n) {
...
nb::bytes whisper_token_to_str_wrapper(whisper_context_wrapper *ctx_w,
                                       whisper_token token) {
...
nb::bytes whisper_token_to_bytes_wrapper(struct whisper_context_wrapper *ctx_w,
                                         whisper_token token) {
  const char *str = whisper_token_to_str(ctx_w->ptr, token);
  size_t l = strlen(str);
...
whisper_token
whisper_token_prev_wrapper(struct whisper_context_wrapper *ctx_w) {
  return whisper_token_prev(ctx_w->ptr);
}

whisper_token
whisper_token_solm_wrapper(struct whisper_context_wrapper *ctx_w) {
...
whisper_token whisper_token_lang_wrapper(struct whisper_context_wrapper *ctx_w,
                                         int lang_id) {
  return whisper_token_lang(ctx_w->ptr, lang_id);
}

whisper_token
whisper_token_translate_wrapper(struct whisper_context_wrapper *ctx_w) {
  return whisper_token_translate(ctx_w->ptr);
}

whisper_token
whisper_token_transcribe_wrapper(struct whisper_context_wrapper *ctx_w) {
...
int whisper_encode_wrapper(struct whisper_context_wrapper *ctx, int offset,
                           int n_threads) {

  return whisper_encode(ctx->ptr, offset, n_threads);
}

int whisper_decode_wrapper(struct whisper_context_wrapper *ctx,
...
int whisper_tokenize_wrapper(struct whisper_context_wrapper *ctx,
                             const char *text, whisper_token *tokens,
                             int n_max_tokens) {
  return whisper_tokenize(ctx->ptr, text, tokens, n_max_tokens);
};

int whisper_lang_auto_detect_wrapper(
    whisper_context_wrapper *ctx, int offset_ms, int n_threads,
    nb::ndarray<nb::numpy, float, nb::shape<-1>, nb::c_contig> lang_probs) {

  return whisper_lang_auto_detect(ctx->ptr, offset_ms, n_threads,
                                  lang_probs.data());
}

int whisper_full_wrapper(
    whisper_context_wrapper *ctx_w, whisper_full_params params,
    nb::ndarray<nb::numpy, float, nb::shape<-1>, nb::c_contig> samples,
    int n_samples) {
...
int whisper_full_parallel_wrapper(
  ...
  return whisper_full_parallel(ctx_w->ptr, params, samples.data(), n_samples,
                               n_processors);
}
...
int64_t whisper_full_get_segment_t0_wrapper(struct whisper_context_wrapper *ctx,
                                            int i_segment) {
  return whisper_full_get_segment_t0(ctx->ptr, i_segment);
}

int64_t whisper_full_get_segment_t1_wrapper(struct whisper_context_wrapper *ctx,
                                            int i_segment) {
  return whisper_full_get_segment_t1(ctx->ptr, i_segment);
}

const nb::bytes
whisper_full_get_segment_text_wrapper(struct whisper_context_wrapper *ctx,
                                      int i_segment) {
...
int whisper_full_n_tokens_wrapper(struct whisper_context_wrapper *ctx,
                                  int i_segment) {
  return whisper_full_n_tokens(ctx->ptr, i_segment);
}

nb::bytes whisper_full_get_token_text_wrapper(whisper_context_wrapper *ctx,
...
whisper_token
whisper_full_get_token_id_wrapper(struct whisper_context_wrapper *ctx,
                                  int i_segment, int i_token) {
  return whisper_full_get_token_id(ctx->ptr, i_segment, i_token);
}

whisper_token_data
whisper_full_get_token_data_wrapper(struct whisper_context_wrapper *ctx,
                                    int i_segment, int i_token) {
  return whisper_full_get_token_data(ctx->ptr, i_segment, i_token);
}

float whisper_full_get_token_p_wrapper(struct whisper_context_wrapper *ctx,
                                       int i_segment, int i_token) {
...
public:
  ...
  WhisperFullParamsWrapper(
      const whisper_full_params &params = whisper_full_params())
    ...
    initial_prompt =
        initial_prompt_str.empty() ? nullptr : initial_prompt_str.c_str();
    suppress_regex =
        suppress_regex_str.empty() ? nullptr : suppress_regex_str.c_str();
    // progress callback
    progress_callback_user_data = this;
    progress_callback = [](struct whisper_context *ctx,
  ...
  WhisperFullParamsWrapper(const WhisperFullParamsWrapper &other)
      : whisper_full_params(
            static_cast<whisper_full_params>(other)), // Copy base struct
...
WhisperFullParamsWrapper
whisper_full_default_params_wrapper(enum whisper_sampling_strategy strategy) {
  return WhisperFullParamsWrapper(whisper_full_default_params(strategy));
}

// callbacks mechanism

void _new_segment_callback(whisper_context *ctx, whisper_state *state,
...
void assign_new_segment_callback(whisper_full_params *params, nb::object f) {

  params->new_segment_callback = _new_segment_callback;
  py_new_segment_callback = std::move(f);
}

bool _encoder_begin_callback(whisper_context *ctx, whisper_state *state,
                             void *user_data) {
...
void assign_encoder_begin_callback(struct whisper_full_params *params,
                                   nb::object f) {
  params->encoder_begin_callback = _encoder_begin_callback;
  py_encoder_begin_callback = f;
}

void _logits_filter_callback(struct whisper_context *ctx,
                             struct whisper_state *state,
                             const whisper_token_data *tokens, int n_tokens,
...
void assign_logits_filter_callback(struct whisper_full_params *params,
                                   nb::object f) {
  params->logits_filter_callback = _logits_filter_callback;
  py_logits_filter_callback = f;
}

void whisper_log_callback(enum ggml_log_level level, const char *text,
                          void *user_data) {
  if (py_whisper_log_callback) {
    nb::gil_scoped_acquire gil;
    py_whisper_log_callback(static_cast<int>(level), std::string(text),
                            nb::none());
  }
}
...
void assign_whisper_log_callback(nb::object f) {
  ...
  whisper_log_set(whisper_log_callback,
                  nullptr); // or replace nullptr if you want user_data
}
...
NB_MODULE(_pywhispercpp, m) {
  ...
  m.def("whisper_init_from_file_with_params", &whisper_init_from_file_wrapper,
        nb::call_guard<nb::gil_scoped_release>(),
        "Various functions for loading a ggml whisper model.\n"
        "Allocate (almost) all memory needed for the model.\n"
        "Return NULL on failure");

  m.def("whisper_init_from_buffer_with_params",
  ...
  m.def("whisper_init_with_params", &whisper_init_wrapper,
        nb::call_guard<nb::gil_scoped_release>(),
        "Various functions for loading a ggml whisper model.\n"
        "Allocate (almost) all memory needed for the model.\n"
        "Return NULL on failure");

  m.def("whisper_free", &whisper_free_wrapper,
        "Frees all memory allocated by the model.");

  m.def("whisper_pcm_to_mel", &whisper_pcm_to_mel_wrapper,
        nb::call_guard<nb::gil_scoped_release>(),
        "Convert RAW PCM audio to log mel spectrogram.\n"
        "The resulting spectrogram is stored inside the provided whisper "
        "context.\n"
        "Returns 0 on success");

  m.def("whisper_set_mel", &whisper_set_mel_wrapper,
        " This can be used to set a custom log mel spectrogram inside the "
        "provided whisper context.\n"
        "Use this instead of whisper_pcm_to_mel() if you want to provide your "
        "own log mel spectrogram.\n"
        "n_mel must be 80\n"
        "Returns 0 on success");

  m.def("whisper_encode", &whisper_encode_wrapper,
        "Run the Whisper encoder on the log mel spectrogram stored inside the "
        "provided whisper context.\n"
        "Make sure to call whisper_pcm_to_mel() or whisper_set_mel() first.\n"
        "offset can be used to specify the offset of the first frame in the "
        "spectrogram.\n"
        "Returns 0 on success");

  m.def("whisper_decode", &whisper_decode_wrapper,
        "Run the Whisper decoder to obtain the logits and probabilities for "
        "the next token.\n"
  ...
  m.def(
      "whisper_tokenize", &whisper_tokenize_wrapper,
  ...
  m.def("whisper_lang_max_id", &whisper_lang_max_id,
        "Largest language id (i.e. number of available languages - 1)");
  m.def("whisper_lang_id", &whisper_lang_id,
        "Return the id of the specified language, returns -1 if not found\n"
        "Examples:\n"
        "\"de\" -> 2\n"
        "\"german\" -> 2");
  m.def("whisper_lang_str", &whisper_lang_str,
        "Return the short string of the specified language id (e.g. 2 -> "
        "\"de\"), returns nullptr if not found");

  m.def("whisper_lang_auto_detect", &whisper_lang_auto_detect_wrapper,
  ...
  m.def("whisper_n_text_ctx", &whisper_n_text_ctx_wrapper,
        "whisper_n_text_ctx");
  m.def("whisper_n_audio_ctx", &whisper_n_audio_ctx_wrapper,
        "whisper_n_audio_ctx");
  m.def("whisper_is_multilingual", &whisper_is_multilingual_wrapper,
        "whisper_is_multilingual");
  m.def("whisper_get_logits", &whisper_get_logits_wrapper,
  ...
  m.def("whisper_token_to_str", &whisper_token_to_str_wrapper,
        "whisper_token_to_str");
  m.def("whisper_token_to_bytes", &whisper_token_to_bytes_wrapper,
  ...
  nb::class_<whisper_full_params>(m, "__whisper_full_params__internal")
      ...
      .def("__repr__", [](const whisper_full_params &self) {
        ...
        oss << "whisper_full_params("
            ...
            << "suppress_regex="
            << (self.suppress_regex ? self.suppress_regex : "None") << ", "
            << "initial_prompt="
            << (self.initial_prompt ? self.initial_prompt : "None") << ", "
            << "prompt_tokens="
            << (self.prompt_tokens ? "(whisper_token *)" : "None") << ", "
            ...
            << "beam_search={beam_size=" << self.beam_search.beam_size
            << ", patience=" << self.beam_search.patience << "}, "
            << "new_segment_callback="
            << (self.new_segment_callback ? "(function pointer)" : "None")
            << ", "
            << "progress_callback="
            << (self.progress_callback ? "(function pointer)" : "None") << ", "
            << "encoder_begin_callback="
            << (self.encoder_begin_callback ? "(function pointer)" : "None")
            << ", "
            << "abort_callback="
            << (self.abort_callback ? "(function pointer)" : "None") << ", "
            << "logits_filter_callback="
            << (self.logits_filter_callback ? "(function pointer)" : "None")
            << ", "
            << "grammar_rules="
            << (self.grammar_rules ? "(whisper_grammar_element **)" : "None")
            << ", "
  ...
  nb::class_<WhisperFullParamsWrapper, whisper_full_params>(
      m, "whisper_full_params")
      .def(nb::init<>())
      ...
      .def_rw("progress_callback",
              &WhisperFullParamsWrapper::py_progress_callback)
      ...
      .def_prop_rw(
          "language",
          [](WhisperFullParamsWrapper &self) { return nb::str(self.language); },
          [](WhisperFullParamsWrapper &self,
             const char *new_c) { // using lang_id let us avoid issues with
                                  // memory management
            const int lang_id =
                (new_c && strlen(new_c) > 0) ? whisper_lang_id(new_c) : -1;
            if (lang_id != -1) {
      ...
      .def_rw("new_segment_callback_user_data",
              &WhisperFullParamsWrapper::new_segment_callback_user_data)
      .def_rw("encoder_begin_callback_user_data",
              &WhisperFullParamsWrapper::encoder_begin_callback_user_data)
      .def_rw("logits_filter_callback_user_data",
              &WhisperFullParamsWrapper::logits_filter_callback_user_data);

  m.def("whisper_full_default_params", &whisper_full_default_params_wrapper);

  m.def("whisper_full", &whisper_full_wrapper,
        nb::call_guard<nb::gil_scoped_release>(),
        "Run the entire model: PCM -> log mel spectrogram -> encoder -> "
        "decoder -> text\n",
        "Uses the specified decoding strategy to obtain the text.\n");

  m.def("whisper_full_parallel", &whisper_full_parallel_wrapper,
        nb::call_guard<nb::gil_scoped_release>(),
        "Split the input audio in chunks and process each chunk separately "
        "using whisper_full()\n"
        "It seems this approach can offer some speedup in some cases.\n"
        "However, the transcription accuracy can be worse at the beginning and "
        "end of each chunk.");

  m.def("whisper_full_n_segments", &whisper_full_n_segments_wrapper,
        nb::call_guard<nb::gil_scoped_release>(),
        "Number of generated text segments.\n"
        "A segment can be a few words, a sentence, or even a paragraph.\n");

  m.def("whisper_full_lang_id", &whisper_full_lang_id_wrapper,
        "Language id associated with the current context");
  m.def("whisper_full_get_segment_t0", &whisper_full_get_segment_t0_wrapper,
        "Get the start time of the specified segment");
  m.def("whisper_full_get_segment_t1", &whisper_full_get_segment_t1_wrapper,
        "Get the end time of the specified segment");

  m.def("whisper_full_get_segment_text", &whisper_full_get_segment_text_wrapper,
        "Get the text of the specified segment");
  m.def("whisper_full_n_tokens", &whisper_full_n_tokens_wrapper,
        "Get number of tokens in the specified segment.");

  m.def("whisper_full_get_token_text", &whisper_full_get_token_text_wrapper,
        "Get the token text of the specified token in the specified segment.");
  m.def("whisper_full_get_token_id", &whisper_full_get_token_id_wrapper,
        "Get the token text of the specified token in the specified segment.");

  m.def("whisper_full_get_token_data", &whisper_full_get_token_data_wrapper,
        "Get token data for the specified token in the specified segment.\n"
        "This contains probabilities, timestamps, etc.");

  m.def("whisper_full_get_token_p", &whisper_full_get_token_p_wrapper,
        "Get the probability of the specified token in the specified segment.");

  ////////////////////////////////////////////////////////////////////////////

  m.def("whisper_bench_memcpy", &whisper_bench_memcpy,
        "Temporary helpers needed for exposing ggml interface");
  m.def("whisper_bench_ggml_mul_mat", &whisper_bench_ggml_mul_mat,
        "Temporary helpers needed for exposing ggml interface");
  ...
  m.def("assign_new_segment_callback", &assign_new_segment_callback,
        "Assigns a new_segment_callback, takes <whisper_full_params> instance "
        "and a callable function with the same parameters which are defined in "
        "the interface",
        nb::arg("params"), nb::arg("callback"));

  m.def("assign_encoder_begin_callback", &assign_encoder_begin_callback,
        "Assigns an encoder_begin_callback, takes <whisper_full_params> "
        "instance and a callable function with the same parameters which are "
        "defined in the interface",
        nb::arg("params"), nb::arg("callback"));

  m.def("assign_logits_filter_callback", &assign_logits_filter_callback,
        "Assigns a logits_filter_callback, takes <whisper_full_params> "
        "instance and a callable function with the same parameters which are "
        "defined in the interface",
        nb::arg("params"), nb::arg("callback"));

  m.def("assign_whisper_log_callback", &assign_whisper_log_callback,
        "Assign a logging callback for whisper_log_set(level: int, message: "
        "str, user_data: None)");
